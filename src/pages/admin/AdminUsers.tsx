...
import { Users, Search, X, Eye, Trash2, Ban, CheckCircle, Edit } from 'lucide-react';
...
export default function AdminUsers() {
    ...
    const mapped: CompanyUser[] = await Promise.all(
      userProfiles.map(async (p: any) => {
        const [productsRes, salesRes] = await Promise.all([
          from('products').select('id', { count: 'exact', head: true }).eq('user_id', p.id),
    ...
    try {
      ...
      if (data.user) {
        await from('profiles').update({
          company_name: newCompany, cnpj: newCnpj, address: newAddress,
          phone: newPhone, plan: newPlan, provider: newProvider,
        }).eq('id', data.user.id);
      }
  ...
  const handleCancelAccount = async () => {
    ...
    await from('profiles').update({ is_active: false, plan: 'free' }).eq('id', cancelUserId);
  ...
  const handleDeleteAccount = async () => {
    ...
    await from('sale_items').delete().in('sale_id',
      (await from('sales').select('id').eq('user_id', deleteUserId)).data?.map((s: any) => s.id) ?? []
    );
    ...
    const productIds = (await from('products').select('id').eq('user_id', deleteUserId)).data?.map((p: any) => p.id) ?? [];
    ...
    await from('profiles').update({ is_active: false, company_name: '[EXCLUÍDA]', plan: 'free' }).eq('id', deleteUserId);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gerencie as empresas do sistema ({users.length} total)</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-muted">
              ...
              <th className="text-left py-3 px-4 font-medium hidden sm:table-cell">Produtos</th>
              <th className="text-left py-3 px-4 font-medium hidden sm:table-cell">Vendas</th>
          ...
          <tbody>
            {filteredUsers.map(u => (
              <tr key={u.id} className="border-t border-border/50 hover:bg-muted/30 transition-colors">
                ...
                <td className="py-3 px-4">
                  <span className={`text-xs px-2 py-1 rounded-full ${u.plan === 'premium' ? 'bg-warning/10 text-warning' : u.plan === 'pro' ? 'bg-info/10 text-info' : u.plan === 'basic' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                    {u.plan || 'free'}
                  </span>
                </td>
                <td className="py-3 px-4 font-mono text-muted-foreground hidden sm:table-cell">{u.total_products ?? 0}</td>
                <td className="py-3 px-4 font-mono text-muted-foreground hidden sm:table-cell">{u.total_sales ?? 0}</td>
                <td className="py-3 px-4">
                ...
                <td className="py-3 px-4 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setDetailUser(u)} title="Detalhes">
                      <Eye className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setEditUser(u)} title="Editar">
                      <Edit className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleToggleActive(u.id, u.is_active)} title={u.is_active ? 'Desativar' : 'Ativar'}>
                      {u.is_active ? <Ban className="w-4 h-4 text-warning" /> : <CheckCircle className="w-4 h-4 text-success" />}
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => setCancelUserId(u.id)} title="Cancelar conta">
                      <X className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => setDeleteUserId(u.id)} title="Excluir dados">
            ...
            {filteredUsers.length === 0 && (
              <tr><td colSpan={7} className="py-8 text-center text-muted-foreground">
                {search || filterPlan !== 'all' || filterStatus !== 'all' ? 'Nenhuma empresa encontrada com esses filtros' : 'Nenhuma empresa cadastrada'}
              </td></tr>
            )}
          </tbody>
        </table>
      ...
      <Dialog open={!!detailUser} onOpenChange={open => { if (!open) setDetailUser(null); }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Informações completas e métricas</DialogDescription>
          </DialogHeader>
          {detailUser && (
            <div className="space-y-4 pt-2">
              ...
              <div className="border-t border-border pt-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="rounded-lg bg-muted p-3 text-center">
                    <p className="text-2xl font-heading font-bold text-primary">{detailUser.total_products ?? 0}</p>
                    <p className="text-xs text-muted-foreground">Produtos</p>
                  </div>
                  <div className="rounded-lg bg-muted p-3 text-center">
                    <p className="text-2xl font-heading font-bold text-info">{detailUser.total_sales ?? 0}</p>
                    <p className="text-xs text-muted-foreground">Vendas</p>
                  </div>
                  <div className="rounded-lg bg-muted p-3 text-center">
                    <p className="text-2xl font-heading font-bold text-success">R$ {(detailUser.total_revenue ?? 0).toFixed(2)}</p>
              ...
              <div className="border-t border-border pt-4 flex items-center justify-between">
                <div>
                  <span className={`text-xs px-2 py-1 rounded-full ${detailUser.is_active ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'}`}>
                    {detailUser.is_active ? 'Ativo' : 'Inativo'}
                  </span>
                  <span className={`ml-2 text-xs px-2 py-1 rounded-full ${detailUser.plan === 'premium' ? 'bg-warning/10 text-warning' : 'bg-muted text-muted-foreground'}`}>
                    {detailUser.plan || 'free'}
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
                  Desde {new Date(detailUser.created_at).toLocaleDateString('pt-BR')}
                </p>
              </div>
            </div>
          )}
      ...
      <AlertDialog open={!!cancelUserId} onOpenChange={open => { if (!open) setCancelUserId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cancelar Conta</AlertDialogTitle>
            <AlertDialogDescription>
              Isso vai desativar a conta e reverter o plano para gratuito. A empresa não poderá mais acessar o sistema. Deseja continuar?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Voltar</AlertDialogCancel>
            <AlertDialogAction onClick={handleCancelAccount} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Cancelar Conta
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={!!deleteUserId} onOpenChange={open => { if (!open) setDeleteUserId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <strong className="text-destructive">ATENÇÃO:</strong> Isso irá excluir PERMANENTEMENTE todos os dados da empresa: produtos, variantes, vendas, logs, alertas, categorias, cores e tamanhos. Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Voltar</AlertDialogCancel>
